...
def mock_gerar_saida_simples(imagem_bytes: bytes, nome_arquivo: str) -> SaidaSimples:
    """Simula o processamento de uma única imagem."""
    time.sleep(2) # Simula o tempo de processamento
    return SaidaSimples(
        markdown=f"## Markdown para: {nome_arquivo}\n\n- Conteúdo da imagem processado.",
        latex=f"\\section*{{LaTeX para: {nome_arquivo}}}\n\nConteúdo da imagem em formato LaTeX."
    )

def mock_ordenar_e_unificar_imagens(imagens: list) -> SaidaUnificada:
    """Simula a ordenação e unificação de múltiplas imagens."""
    time.sleep(5) # Simula um processamento mais longo
    nomes_arquivos = [img.name for img in imagens]
    ordem_reversa = list(reversed(nomes_arquivos)) # Simula uma ordem inferida
    
    # Simula a junção dos conteúdos
    markdown_unificado = f"# Documento Unificado\n\nOrdem inferida: {' -> '.join(ordem_reversa)}\n\n"
    latex_unificado = "\\documentclass{article}\n\\begin{document}\n"
    
    for nome in ordem_reversa:
        markdown_unificado += f"## Conteúdo de {nome}\n\nTexto extraído da imagem {nome}.\n\n---\n"
        latex_unificado += f"\\section*{{Conteúdo de {nome}}}\n\nTexto extraído da imagem {nome}.\n\n"
        
    latex_unificado += "\\end{document}"
    
    return SaidaUnificada(
        markdown_completo=markdown_unificado,
        latex_completo=latex_unificado,
        ordem_inferida=ordem_reversa
    )        